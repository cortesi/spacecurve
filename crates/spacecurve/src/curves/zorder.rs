...
#[derive(Debug)]
pub struct ZOrder<C: Coord, I: Index> {
...
impl<C: Coord, I: Index> ZOrder<C, I> {
...
impl<C: Coord, I: Index> SpaceCurve for ZOrder<C, I> {
    type Coord = C;
    type Index = I;
...
mod tests {
    ...
    fn from_dimensions_guard() {
        ...
        assert!(ZOrder::<u32, u32>::from_dimensions(2, 1u32 << 16).is_err());
        // 2 * 15 = 30 bits total → ok
        assert!(ZOrder::<u32, u32>::from_dimensions(2, 1u32 << 15).is_ok());
        // 4 * 8 = 32 bits total → reject
        assert!(ZOrder::<u32, u32>::from_dimensions(4, 1u32 << 8).is_err());
        // 4 * 7 = 28 bits total → ok
        assert!(ZOrder::<u32, u32>::from_dimensions(4, 1u32 << 7).is_ok());
    }

    #[test]
    fn sequence_matches_reference_2d() {
        let curve = ZOrder::<u32, u32>::from_dimensions(2, 4).unwrap();
        let expected = vec![
            vec![0, 0],
            vec![1, 0],
            vec![0, 1],
    ...
    fn roundtrip_holds_for_small_cases() {
        let curve = ZOrder::<u32, u32>::from_dimensions(3, 4).unwrap();
    ...
    fn roundtrip_dims_up_to_four() {
        for dim in 1..=4 {
            let curve = ZOrder::<u32, u32>::from_dimensions(dim, 2).unwrap();
            for i in 0..curve.length() {
                let point = curve.point(i);
                assert_eq!(
                    curve.index(&point),
